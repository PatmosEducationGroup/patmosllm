...
export async function GET() {
  try {
    ...
    if (!user) {
      logger.debug({ operation: 'GET /api/user/profile' }, 'No authenticated user found')
    ...
    logger.debug({ userId: user.id, email: user.email }, 'Profile API user authenticated')
    ...
    logger.debug({ userId: user.id, hasDeletedAt: !!profile.deleted_at }, 'Profile API data fetched')
